...
public class Formatter {
   ...
   * DefaultCodeFormatterOptions</code> class. Supported types are boolean, int, long, and string.
  ...
  void modifyCFOptions(DefaultCodeFormatterOptions cfOptions) {
    for (Map.Entry<String, Object> e : codeFormatterOptionsMap.entrySet()) {
      ...
      try {
        Field f = DefaultCodeFormatterOptions.class.getField(fieldName);
  ...
  /**
   * Formats all files in the given directory. Formatted files are written to new files starting
   * with "formatted_".
   *
   * <p>EDIT: They now write to the original file
   *
    ...
    for (File f : files) {
      try {
        ...
        throw new RuntimeException(
            "Exception at file: " + f.getParentFile() + "/" + f.getName(), e);
      }
    }
  }
   ...
   */
  public String format(String code) throws MalformedTreeException, BadLocationException {
